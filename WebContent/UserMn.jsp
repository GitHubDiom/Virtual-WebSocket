				...
				<ul class="nav ace-nav" style="height: auto">
					<li class="grey"><a data-toggle="dropdown"
						class="dropdown-toggle" href="#"> <i class="icon-tasks"></i> <span
							class="badge badge-grey">21</span>
					</a>
					...
					<li class="purple"><a data-toggle="dropdown"
						class="dropdown-toggle" href="#"> <i
							...
							class="badge badge-important">18</span>
					</a>
...
<script>
	...
	//$("#sample-table-2").hide();
	function search() {
		$("#sample-table-2").find("tbody").empty();
		$
				.ajax({
					url : "http://localhost:8080/WebProject2/UsersServlet",
					type : "get",
					...
					success : function(data) {
						...
						$
								.each(
										...
										function(i, item) {
											$(
													"<tr><td><input type='checkbox' value="+i+"></td><td>"
															+ item.phone
															+ "</td><td>"
															+ item.userName
															+ "</td><td>"
															...
															+ "</td><td>"
															+ item.email
															+ "<td align='center'><button class='btn btn-xs btn-info' onclick='updateFunction("
															+ i
	...
	function updateFunction(i) {
		...
		console.info($("#sample-table-2 tr").eq(i + 1));
		var tds = $("#sample-table-2 tr").eq(i + 1).find('td');
		var phone = tds.eq(1).html();
		var username = tds.eq(2).html();
		var pwd = tds.eq(3).html();
		var email = tds.eq(4).html();
		//  $("<tr> <td>1</td> <td>2</td> <td>3</td> <td>4</td> </tr>").appendTo( $("#modal-table tbody") );

		$(
				"<tr><td><input disabled=\"disabled\"  id='getPhone' style='width:100%' value='"
						+ phone
						+ "' /></td><td><input   id='getName' style='width:100%' value='"
						+ username
						+ "' /></td> <td><input id='getPwd' style='width:100%' value='"
						+ pwd
						+ "' /></td> <td><input id='getEmail' style='width:100%' value='"
						+ email + "' /></td> </tr>").appendTo(
				$("#modal-table tbody"));
	...
	function deleteFunction(i) {
		var uerName = $("#sample-table-2 tr").eq(i + 1).find("td").eq(2).html();
		alert(uerName);
		//return;
		$.ajax({
			url : "http://localhost:8080/WebProject2/UserDeleteServlet",
			type : "post",
			async : true,//trueæ¯å¼æ­¥ï¼é»è®¤ï¼ï¼falseæ¯åæ­¥
			data : {
				"uerName" : uerName,
...
<script>
	function upgradFunction(i) {
		$.ajax({
			url : "http://localhost:8080/WebProject2/upGradeServlet",
			type : "post",
			async : true,//trueæ¯å¼æ­¥ï¼é»è®¤ï¼ï¼falseæ¯åæ­¥
			data : {
				"phone" : $("#getPhone").val(),

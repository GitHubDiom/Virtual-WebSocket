	...
	Ext.onReady(function() {//下面就是页面加载完成后开始执行的代码,对这行代码的说明：之所以将该行代码放在这里，是因为为了使“当前浏览器不支持HTML5 WebSocket！！！”会话框好使。
		if (window.WebSocket) {
			var initWebSocket = function () {//初始话WebSocket
				websocket = new WebSocket(encodeURI("ws://localhost:8080/WebProject2/message?name='"
			...
			Ext.define('MessageContainer',{//用于展示用户的聊天信息
				...
				tpl : [
						'<div class="l-im-message-warn">​您正在进行群聊模式</br>交谈中请勿轻信汇款、中奖信息、陌生电话。 请遵守相关法律法规。 </div>',
						'<tpl for=".">',
				...
				receive : function(message) {//将服务器推送的信息展示到页面中
					var me = this;
					message['timestamp'] = Ext.Date.format(new Date(message['timestamp']),'H:i:s');//获取时间
